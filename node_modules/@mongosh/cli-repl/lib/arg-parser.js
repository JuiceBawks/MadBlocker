...
function parseCliArgs(args) {
    ...
    const parsed = yargs_parser_1.default(programArgs, OPTIONS);
    const positionalArguments = (_a = parsed._) !== null && _a !== void 0 ? _a : [];
    for (const arg of positionalArguments) {
        if (arg.startsWith('-')) {
            throw new Error(`  ${clr_1.colorizeForStderr(i18n_1.default.__(UNKNOWN), ['red', 'bold'])} ${clr_1.colorizeForStderr(String(arg), 'bold')}
        ${constants_1.USAGE}`);
        }
    }
...
function verifyCliArguments(args) {
    for (const unsupported of UNSUPPORTED_ARGS) {
        if (unsupported in args) {
            throw new errors_1.MongoshUnimplementedError(`Argument --${unsupported} is not yet supported in mongosh`, errors_1.CommonErrors.InvalidArgument);
        }
    }
    const messages = [];
