...
let ShellConfig = class ShellConfig extends decorators_1.ShellApiClass {
    ...
    async set(key, value) {
        var _a, _b;
        helpers_1.assertArgsDefinedType([key], ['string'], 'config.set');
    ...
    async get(key) {
        var _a, _b;
        helpers_1.assertArgsDefinedType([key], ['string'], 'config.get');
        const { evaluationListener } = this._instanceState;
...
let ShellApi = class ShellApi extends decorators_1.ShellApiClass {
    ...
    async exit(exitCode) {
        var _a, _b;
        helpers_1.assertArgsDefinedType([exitCode], [[undefined, 'number']], 'exit');
        helpers_1.assertCLI(this._instanceState.initialServiceProvider.platform, 'the exit/quit commands');
    ...
    async Mongo(uri, fleOptions, otherOptions) {
        helpers_1.assertCLI(this._instanceState.initialServiceProvider.platform, 'new Mongo connections');
    ...
    async connect(uri, user, pwd) {
        helpers_1.assertArgsDefinedType([uri, user, pwd], ['string', [undefined, 'string'], [undefined, 'string']], 'connect');
        helpers_1.assertCLI(this._instanceState.initialServiceProvider.platform, 'new Mongo connections');
    ...
    async load(filename) {
        helpers_1.assertArgsDefinedType([filename], ['string'], 'load');
        ...
        context.__dirname = path_1.dirname(resolvedFilename);
        ...
        try {
            ...
            if (previousFilename) {
                ...
                context.__dirname = path_1.dirname(previousFilename);
            }
            else {
    ...
    async sleep(ms) {
        return await util_1.promisify(setTimeout)(ms);
    }
    async print(...origArgs) {
        var _a;
        const { evaluationListener } = this._instanceState;
        const args = await Promise.all(origArgs.map(arg => decorators_1.toShellResult(arg)));
...
__decorate([
    ...
    decorators_1.shellCommandCompleter(useCompleter)
], ShellApi.prototype, "use", null);
__decorate([
    decorators_1.directShellCommand,
    decorators_1.returnsPromise,
    decorators_1.shellCommandCompleter(showCompleter)
